...
extern char** MapStrPtr_keys(MapStrPtr_t* in){
    ...
    int len = MapStrPtr_length(m);
    
	/* On empty hashmap, return immediately an array containing only a null pointer */
	if (len <= 0)
		return (char** ) calloc(1,sizeof(char*));	

    /* Otherwise we prepare the output array */
	...
	for(i = 0; i< m->table_size; i++)
    {
		if(m->data[i].in_use != 0) 
        {
			out[pos] = strdup(m->data[i].key);
            pos++;
		}
    }
    
    /* Set the last to be a null pointer */
    out[len] = NULL;
